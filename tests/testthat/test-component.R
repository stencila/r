...
test_that('Component read errors correctly', {
  c = Component$new('foo/bar');
  expect_equal(c$path(), 'foo/bar', 'Initial path is set and doesnt matter if it doesnt exist')
  #expect_equal(throws(c$read.bind(c), /Filesystem path does not exist/, 'Explicit read should error')
});
...
test_that('Component writer creates the right directories', {
  ...
  c = Component$new(file)
  expect_equal(c$path(), file, 'Path is set')
  c$write()
